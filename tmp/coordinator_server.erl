...
server_loop(State) ->
  ...
  NewState = receive
		 ...
		 {CallingPid,who_is_leader}->  io:format("who_is_leader ~p~n", [{CallingPid,Coordinator,?MODULE,?LINE}]),
					       CallingPid!{ok,Coordinator};
...
who_is_leader(Timeout,Node)->
    {?COORDINATOR_SERVER, Node}!{self(),who_is_leader},
    Result=receive
	       {ok,LeaderNode}->
		   {ok,LeaderNode}
	   after
